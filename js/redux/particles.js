...
function ReadDomain(e, name, edit_type)
{
	...
	function ReadVec(name)
	{
		switch(edit_type)
		{
			...
			default: d.ReadVec3(name); print("unknown domain type " + type);
		}
	}
...
function ReadActionList(effect)
{
	...
	for(var k = 0; k < acount; k++)
	{
		...
		switch(atype)
		{
			...
			case 1: // Bounce
				a.ReadFP32("friction")
				a.ReadFP32("friction_var")
				a.ReadFP32("resilience")
				a.ReadFP32("resilience_var")
				a.ReadFP32("cutoff")
				ReadDomain(a, "domain", 0)	
				a.ReadBool("dbg_draw")	
			break;
			...
			case 57: // Aux Animator (NEW in Redux !!)
...
for(var i = 0; effects.More(); i++)
{
	...
	effect.ReadFP32("ext_shader_param") // NEW in Redux !!
	...
	effect.ReadBool("fl_adjust_by_aspect") // NEW in Redux !!
	...
	effect.ReadHintStr("shader_name", "choose") // Redux: changed 'shader, str_shared' to 'choose'
